...
class Contact(models.Model):
    ...
    cel = models.CharField(max_length=10, null=False, blank=False, help_text='Escribir el número sin guiones (-).', validators=[validators.MaxLengthValidator(limit_value=10, message=format_html('<div class="alert alert-warning alert-dismissible fade show" role="alert"> Tu número de teléfono no es válido. <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>'))])
    ...
    img_contact = models.ImageField(upload_to="profile/", default="profile/profile.png", blank=True, null=True, validators=[validators.FileExtensionValidator(allowed_extensions=['jpg', 'png', 'jpeg'], message=format_html('<div class="alert alert-warning alert-dismissible fade show" role="alert"> Solo acepta archivos png y jpg <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>'))])
